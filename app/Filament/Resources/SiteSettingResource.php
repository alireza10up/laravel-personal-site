        ...
        return $form
            ->schema([
                ...
                Forms\Components\Repeater::make('what_i_do')
                    ->schema([
                        Forms\Components\TextInput::make('title')->required()->rules(['required', 'string', 'max:128']),
                        Forms\Components\FileUpload::make('icon')->required()->directory('icons')->rules(['required', 'image', 'max:2048']),
                        Forms\Components\Textarea::make('description')->required()->rules(['required', 'string', 'max:2048']),
                    ])
                    ->required()
                    ->label('What I Do'),
